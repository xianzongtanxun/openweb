...
#pragma warning(disable:4786)
	...
	static std::map<HWND, CWndShadow *> s_Shadowmap;             //通过绑定窗体来加入MAP
#pragma warning(pop) 
	...
	enum ShadowStatus
	{
		...
		SS_PARENTVISIBLE = 1<< 2,	// Parent window is visible, if not, the above one is always false (父窗体可见)
		SS_DISABLEDBYAERO = 1 << 3	// Shadow is enabled, but do not show because areo is enabled  （）
	};
	BYTE m_Status;

	unsigned char m_nDarkness;	// Darkness, transparency of blurred area   模糊区的透明度
	unsigned char m_nSharpness;	// Sharpness, width of blurred border of shadow window  锐利度，阴影边界模糊的窗口宽度
	signed char m_nSize;	// Shadow window size, relative to parent window size       虚窗宽度
