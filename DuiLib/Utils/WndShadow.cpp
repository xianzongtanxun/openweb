...
void CWndShadow::Create(HWND hParentWnd)
{
	...
	if(NULL == s_UpdateLayeredWindow)  //透明函数不存在，虚框
		return;

	// Already initialized
	_ASSERT(s_hInstance != INVALID_HANDLE_VALUE);
	if(INVALID_HANDLE_VALUE == s_hInstance)
		return;
...
#pragma warning(default: 4312)
	...
	case WM_SIZE:
		if(pThis->m_Status & SS_ENABLED && !(pThis->m_Status & SS_DISABLEDBYAERO))//enable
		{
			//if(SIZE_MAXIMIZED == wParam || SIZE_MINIMIZED == wParam)
			if(SIZE_MINIMIZED == wParam)		//最大化改变了大小，需要重绘???  (窗体最大化，应该不用去绘画的，最小化也不需要重新绘制的)
			{
				ShowWindow(pThis->m_hWnd, SW_HIDE);
				pThis->m_Status &= ~SS_VISABLE;//窗体设置为不可见
			}
			else
			{
				...
				if(WS_VISIBLE & lParentStyle)	// Parent may be resized even if invisible
				{
					...
					if(!(pThis->m_Status & SS_VISABLE))//加入当前窗体不可见，则进行刷新可见。
					{
...
void CWndShadow::Update(HWND hParent)
{
	...
	//add by liang.tt 2014-09-03 防止CreateDIBSection返回失败,pvBits为null导致ZeroMemory发生crash
	if (hbitmap == NULL)
	{
		return;
	}
...
void CWndShadow::MakeShadow(UINT32 *pShadBits, HWND hParent, RECT *rcParent)
{
	...
	int (*ptAnchors)[2] = new int[nAnchors + 2][2];//二维数组
...
{
	if(NewSize > 20 || NewSize < -20)//为什么宽度不能超过20？？？
		return false;
...
{
	if(NewSharpness > 20)   //阴影边界模糊的窗口宽度 不能超过20????
		return false;
...
{
	NewDarkness %= 256;
	//if(NewDarkness > 255)  // 模糊区的透明度不能大于255
	//	return false;
...
bool CWndShadow::SetPosition(int NewXOffset, int NewYOffset)
{
	if(NewXOffset > 20 || NewXOffset < -20 ||
		NewYOffset > 20 || NewYOffset < -20)  //偏移范围
		return false;
...
void CWndShadow::Show(HWND hParentWnd)
{
	...
	{
		// Determine the show state of shadow according to parent window's state(根据父窗口的状态确定阴影的显示状态)
