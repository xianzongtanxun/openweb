...
namespace DuiLib
{
	...
	CEditUI::CEditUI() : m_pWindow(NULL), m_uMaxChar(255), m_bReadOnly(false), 
		...
		m_dwEditbkColor(0xFFFFFFFF), m_dwPlaceTextColor(0xFFFFFFFF), m_iWindowStyls(0), m_iBkFont(0), m_placeholder(_T(""))
	{
	...
	void CEditUI::PaintText(HDC hDC)
	{
		...
		if( m_sText.IsEmpty() && m_placeholder.IsEmpty()) return;
		...
		if (sText.IsEmpty() && !m_placeholder.IsEmpty())
		{
			if (m_iBkFont == 0 && m_iBkFont != m_iFont){
				m_iBkFont = m_iFont;
			}
			CRenderEngine::DrawText(hDC, m_pManager, rc, m_placeholder, m_dwPlaceTextColor, \
				m_iBkFont, DT_SINGLELINE | m_uTextStyle);
			return;
		}
		if (sText.IsEmpty()){
			return;
		...
		if (IsEnabled()) 
		{
			if (m_bShowHtml)
			{
				...
				CRenderEngine::DrawHtmlText(hDC, m_pManager, rc, sText, m_dwTextColor, \
					NULL, NULL, nLink, m_iFont, DT_SINGLELINE | m_uTextStyle);
			}
			else
			{
	...
	void CEditUI::SetBkFont(int iFont)
	{
		if (m_iBkFont == iFont) {
			return;
		}
	...
	void CEditUI::SetPlaceholder(LPCTSTR tText)
	{
		if (m_placeholder == tText) {
			return;
		}
		m_placeholder = tText;
		Invalidate();
	...
	void CEditUI::SetBkTextColor(DWORD color)
	{
		if (m_dwBackColor == color) {
			return;
		}
		m_dwEditbkColor = color;
		Invalidate();
	}

	void CEditUI::SetPlaceTextColor(DWORD color)
	{
		if (m_dwPlaceTextColor == color) {
			return;
		}
		m_dwPlaceTextColor = color;
		Invalidate();
	}
}